...
public:
    ...
    int getMaxMinDistBacktracking(vector<vector<int>> &grid, int i, int j, vector<vector<bool>> &visited) {
        int n = grid.size();
    ...
    bool checkMinDistPossibleDfs(vector<vector<int>> &grid, int i, int j, vector<vector<bool>> &visited, int minDist) {
        int n = grid.size();
    ...
    bool checkMinDistPossibleBfs(vector<vector<int>> &grid, vector<vector<bool>> &visited, int minDist) {
        int n = grid.size();
    ...
    int fillDistance(vector<vector<int>> &grid) {
        int n = grid.size();
    ...
    int maximumSafenessFactor(vector<vector<int>>& grid) {
        ...
        while (minDist <= maxDist) {
            ...
            // BFS - AC (Beats 30%)
            if (checkMinDistPossibleBfs(grid, visited, mid)) {
                ans = mid;
                minDist = mid + 1;
            } else {
                maxDist = mid - 1;
